...
; (function ($, window, document, undefined) {
    ...
    Plugin.prototype = {
        ...
        pixelX: function (val, i) {
            ...
            return ((graph.width() - config.xPadding) / config.data[i].values.length) * val + (config.xPadding * 1.25);
        },
        pixelY: function (val) {
            config = this.options;
            var graph = $(this.element).find('.SimpleChartcanvas');
            return graph.height() - (((graph.height() - config.yPadding - 8) / this.FindYMax()) * val) - config.yPadding;
        },
        ...
        drawAxis: function (c, graph) {
            ...
            for (var i = 0; i < config.data.length; i++) {
                for (var j = 0; j < config.data[i].values.length; j++) {
                    if (xelementarray.indexOf(config.data[i].values[j].X) < 0) {
						...
						if (j % 7 == 0) {c.fillText(config.data[i].values[j].X, that.pixelX(j, i), graph.height() - config.yPadding + 20);}
						c.beginPath();
						c.moveTo(that.pixelX(j, i), graph.height() - config.yPadding - 3);
						c.lineTo(that.pixelX(j, i), graph.height() - config.yPadding + 3);
						c.stroke();
                    }
                }
            }
			...
			incrementvalue = parseInt(maxY / 5);
            for (var i = 0; i <= maxY ; i += incrementvalue) {
                c.fillStyle = config.textcolor;
                c.fillText(i, config.xPadding - 10, that.pixelY(i));
				if (i > 0) {
					c.save();
					c.beginPath();
					c.moveTo(config.xPadding, that.pixelY(i));
					c.lineTo(graph.width(), that.pixelY(i));
					c.lineWidth = 1;
					...
					c.restore();
					c.beginPath();
					c.moveTo(config.xPadding - 3, that.pixelY(i));
					c.lineTo(config.xPadding + 3, that.pixelY(i));
					c.lineWidth = 2;
					c.strokeStyle = config.axiscolor;
					c.stroke();
				}
        ...
        drawLineAreaScatteredHybridCharts: function (c, graph) {
            ...
            function drawToolTiponHover(e) {
                ...
                for (var i = 0; i < linepoints.length; i++) {
                    ...
                    var dx = mouseX - dot.x;
                    var dy = mouseY - dot.y + 3;
					var nb = parseInt(dot.tip);
					var nb_leg = (nb < 2)?config.title.slice(0,-1):config.title;
                    if (dx * dx + dy * dy < dot.rXr) {
                        ...
                        tipCtx.fillText(dot.tipX, 4, 15);
						tipCtx.fillText(nb+" "+nb_leg.toLowerCase(), 3, 30);
